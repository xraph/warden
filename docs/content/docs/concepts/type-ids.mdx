...
> **Auto-assigned by default.** Every store's `Create*` method generates a TypeID for you when the input's `ID` field is nil — see the auto-ID note in [Roles & Permissions](/docs/authorization/roles-permissions). The DSL goes further: `.warden` source never references TypeIDs at all. Roles cite each other by slug (`role editor : viewer`), permissions by name (`grants = ["doc:read"]`), namespaces by path (`/eng/admin`). TypeIDs are an implementation detail you only see when you're calling the store directly or inspecting the database.
